...
function RecipeDetail() {
  ...
  const steps = data.analyzedInstructions.length > 0 ? data.analyzedInstructions[0].steps : [];
...
function handleClick (recipe) {
  ...
  return (
    <StyledMain>
            ...
            <StyledUl>
                {
                    ingredients && ingredients.map((ingredient) => (
                        <StyledLi key={ingredient.id}>{ingredient.name}</StyledLi>
                    ))}
            </StyledUl>
        </Container>
        <Container>
        ...
        <StyledLink to={`/`} key={data.id} onClick={() => handleClick(recipe)}>Back</StyledLink>
    </StyledMain>
  );
}
